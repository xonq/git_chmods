...
#https://github.com/rajatdiptabiswas/tetris-pygame/blob/master/Tetris.py
...
class Piece(object):
    def __init__(self, x, y, shape, colors = None):
        self.x = x
        self.y = y
        self.shape = shape
        if colors:
            ...
            self.color = [random.choice(shape_colors)
                for x in range(''.join(shape[0]).count('0'))]  # choose color from the shape_color list
        print(self.color)
...
def clear_rows(grid, locked):
    ...
    contig = None
    increment = 0
    for i in range(len(grid) - 1, -1, -1):      # start checking the grid backwards #loops through the row going backwards
        grid_row = grid[i]                      # get the last row #set row to every row in our grid
        if (0, 0, 0) not in grid_row:           # if there are no empty spaces (i.e. black blocks) #if 000 doesn't exist, it means it's filled
            print(grid_row)

           # draw_text_left(str(grid_row), 40, (255,255,255), surface)
            eval_clear = True
            print('evaluating')
            for j in range(1,len(grid_row)-2,2):
                if grid_row[j] != grid_row[j+1]:
                    print(grid_row[j], grid_row[j+1])
                    eval_clear = False
                    break
            if eval_clear:
                contig = grid_row
              ...
              # add positions to remove from locked
                ...
                for j in range(len(grid_row)): #to get through every row, we need to change j and get through every row. since lockedji is a dictionary, we can delete the ro
                    try:
...
#for every key in our sorted locked position based on our y value; key = lamda x sorts it base on y
            x, y = key
            if y < index:                       # if the y value is above the removed index
                new_key = (x, y + increment)    # shift position to down
                locked[new_key] = locked.pop(key)

    return increment, contig
...
def draw_next_shape(piece, surface):
    ...
    for i, line in enumerate(shape_format):
        ...
        for j, column in enumerate(row):
            if column == '0':
                pygame.draw.rect(surface, piece.color[color_counter], (start_x + j*block_size, start_y + i*block_size, block_size, block_size), 0)
                color_counter += 1
    surface.blit(label, (start_x, start_y - 30))

    # pygame.display.update()
def draw_contig(contig, surface):
    font = pygame.font.Font(fontpath, 30)
    label = font.render(str([str(x) for x in contig]), 1, (255, 255, 255))
...
def draw_window(surface, grid, score=0, last_score=0):
    ...
    label = font.render('CONTIG GO', 1, (255, 255, 255))  # initialise 'Tetris' text with white

    surface.blit(label, ((top_left_x + play_width/ 2) - (label.get_width() / 2), 30))  # put surface on the center of the window

    # current score
    font = pygame.font.Font(fontpath, 30)
    label = font.render('contigs ' + str(score) , 1, (255, 255, 255))
    ...
    # last score
    label_hi = font.render(' ', 1, (255, 255, 255))
...
def main(window):
    ...
    while run:
        ...
        if fall_time / 1000 > fall_speed:
            fall_time = 0
            current_piece.y += 1
            if not valid_space(current_piece, grid) and current_piece.y > 0: #were not at the top of the screen
                current_piece.y -= 1 #moved into an invalid position and move back up
                ...
                change_piece = True #why True? if we are moving down into a position down that's not valid, so we must of hit another piece or the bottom. we will return to changing piece that will come down the screen
        ...
        for i in range(len(piece_pos)):
            x, y = piece_pos[i]
            if y >= 0:
                grid[y][x] = current_piece.color[color_counter]
                color_counter += 1
        if change_piece:  # if the piece is locked
            color_counter = 0
            for pos in piece_pos:
                p = (pos[0], pos[1])
                locked_positions[p] = current_piece.color[color_counter]       # add the key and value in the dictionary
                color_counter += 1
            current_piece = next_piece
            next_piece = get_shape()
            change_piece = False
            count, contig = clear_rows(grid, locked_positions) 
            score += count  #clear_rows(grid, locked_positions) * 1    # increment score by 10 for every row cleared
            update_score(score)
        ...
        if contig is not None:
            draw_contig(contig, window)
        pygame.display.update()

        if check_lost(locked_positions):
            run = False

    draw_text_middle('You have sequence!', 40, (255, 255, 255), window)
    pygame.display.update()
...
if __name__ == '__main__':
    ...
    pygame.display.set_caption('contig go')
