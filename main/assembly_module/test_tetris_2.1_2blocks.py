        ...
        if (0, 0, 0) not in grid_row:           # if there are no empty spaces (i.e. black blocks) #if 000 doesn't exist, it means it's filled
            ...
            if eval_clear:
              ...
              # add positions to remove from locked
                ...
                for j in range(len(grid_row)): #to get through every row, we need to change j and get through every row. since lockedji is a dictionary, we can delete the row.
                    try:
...
def main(window):
    ...
    while run:
        ...
        if change_piece:  # if the piece is locked
            ...
            next_piece = get_shape()
            change_piece = False
            score += clear_rows(grid, locked_positions) * 1    # increment score by 10 for every row cleared
            update_score(score)
    ...
    draw_text_middle('You made a genome!', 40, (255, 255, 255), window)
    pygame.display.update()
